...
export const handleQuestionCommand = async (
    ...
    actions.forEach(async (action) => {
      switch (action.name) {
        case Actions.INSERT_QUESTION:
          ...
          let answerOptions = action.data
            .answerOptions as Array<AnswerOptionData>;

          await trx
            .insertInto('answerOptions')
            .values(
              answerOptions.map((ao: AnswerOptionData) => ({
                id: ao.id as string,
            ...
            .onConflict((oc) =>
              oc.column('id').doUpdateSet((eb) => {
                const keys = Object.keys(
                  answerOptions[0]!
                ) as (keyof AnswerOptionData)[];
                return Object.fromEntries(
                  keys.map((key) => [key, eb.ref(key)])
                );
              })
            )
            .execute();
